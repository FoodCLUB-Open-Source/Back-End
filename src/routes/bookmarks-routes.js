...
router.get("/:user_id", rateLimiter(), inputValidator, async (req, res, next) => {
  try {
    ...
    SELECT 
        p.id, p.title, p.description, p.video_name, p.thumbnail_name, p.created_at, 
        u.id AS user_id, u.full_name, u.profile_picture, u.username, pc.category_name AS post_category,
        CASE 
